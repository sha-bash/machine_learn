...
w = np.array([0.0, 0.0, 1.0])  # начальные весовые коэффициенты
nt = 0.0005  # шаг сходимости SGD
lm = 0.01  # скорость "забывания" для Q
N = 500  # число итераций SGD

Q = np.mean(log_loss(w, x_train, y_train))  # показатель качества
Q_plot = [Q]

k = np.random.randint(0, n_train - 1, size=N)  # случайные индексы
ek = log_loss(w, x_train[k], y_train[k])  # вычисление потерь для выбранных векторов
w = w - nt * np.mean(dlog_loss(w, x_train[k], y_train[k]), axis=0)  # корректировка весов по SGD
...
line_x = np.linspace(0, np.max(x_train[:, 0]), num=100)  # формирование графика разделяющей линии
...
plt.axis([0, np.max(x_train[:, 0]), 0, np.max(x_train[:, 1])])  # установка пределов осей x и y
plt.ylabel("длина")
plt.xlabel("ширина")
plt.grid(True)
plt.show()
